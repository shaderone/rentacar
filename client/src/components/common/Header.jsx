...
function Header() {
    ...
    // Helper to determine where the Logo should point
    const getHomeLink = () => {
            ...
            <header className='app-container mx-auto bg-white/90 dark:bg-slate-900/90 backdrop-blur-xl rounded-[2rem] px-6 py-4 flex justify-between items-center shadow-lg shadow-slate-200/50 dark:shadow-black/50 border border-white/50 dark:border-slate-800 transition-all duration-300'>
                ...
                <nav className='hidden md:flex items-center gap-8 font-bold text-xs tracking-widest uppercase'>
                    ...
                    {user?.role === 'admin' ? (
                        /* --- ADMIN LINKS --- */
                        <>
                            <Link to='/admin/dashboard' className={`transition duration-300 hover:text-emerald-600 dark:hover:text-emerald-400 ${isActive('/admin/dashboard') ? 'text-emerald-600 dark:text-emerald-400' : 'text-slate-500 dark:text-slate-400'}`}>
                                Dashboard
                            </Link>
                            <Link to='/admin/users' className={`transition duration-300 hover:text-emerald-600 dark:hover:text-emerald-400 ${isActive('/admin/users') ? 'text-emerald-600 dark:text-emerald-400' : 'text-slate-500 dark:text-slate-400'}`}>
                                Users
                            </Link>
                            <Link to='/admin/cars' className={`transition duration-300 hover:text-emerald-600 dark:hover:text-emerald-400 ${isActive('/admin/cars') ? 'text-emerald-600 dark:text-emerald-400' : 'text-slate-500 dark:text-slate-400'}`}>
                                All Cars
                            </Link>
                        </>
                    ) : user?.role === 'host' ? (
                        /* --- HOST LINKS --- */
                        <>
                            <Link to='/host/dashboard' className={`transition duration-300 hover:text-emerald-600 dark:hover:text-emerald-400 ${isActive('/host/dashboard') ? 'text-emerald-600 dark:text-emerald-400' : 'text-slate-500 dark:text-slate-400'}`}>
                                Dashboard
                            </Link>
                    ...
                    {user ? (
                        <div className="flex items-center gap-3 pl-4 border-l border-gray-200 dark:border-slate-700">
                            <div className="text-right hidden lg:block">
                                ...
                                <p className="text-[10px] font-bold text-emerald-600 dark:text-emerald-400 uppercase tracking-wide">{user.role}</p>
                            </div>
                            ...
                            <div className="group relative">
                                ...
                                <div className="absolute right-0 top-full mt-2 w-32 bg-white dark:bg-slate-800 rounded-xl shadow-xl border border-gray-100 dark:border-slate-700 p-2 opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all duration-200 transform origin-top-right z-50">
                                    ...
                                    <button onClick={onLogout} className="w-full flex items-center gap-2 px-3 py-2 text-xs font-bold text-red-500 hover:bg-red-50 dark:hover:bg-red-900/20 rounded-lg transition cursor-pointer">
                                        <FaSignOutAlt /> Logout
                                    </button>
                                </div>
                            </div>

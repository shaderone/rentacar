...
function MyBookings() {
    ...
    // --- HELPER: STATUS COLORS (Updated for Dark Mode) ---
                                ...
                                <div className="flex flex-col md:grid md:grid-cols-12 gap-4 md:items-center relative">
                                    ...
                                    <div className="col-span-3 p-4 md:p-0 pr-6 text-right flex items-center justify-end gap-2">
                                        ...
                                        {booking.status === 'Pending' && (
                                            <button className="px-4 py-2 rounded-lg bg-emerald-50 dark:bg-emerald-500/10 text-emerald-600 dark:text-emerald-400 text-xs font-bold border border-emerald-100 dark:border-emerald-500/20 hover:bg-emerald-600 hover:text-white dark:hover:bg-emerald-500 dark:hover:text-white transition flex items-center gap-2">
                                                <FaCheckCircle /> Pay & Confirm
                                            </button>
                                        )}
                                        ...
                                        {(booking.status === 'Pending' || booking.status === 'Confirmed') && (
                                            <button className="px-3 py-2 mr-3 rounded-lg bg-red-50 dark:bg-red-500/10 text-red-500 dark:text-red-400 text-xs font-bold hover:bg-red-500 hover:text-white dark:hover:bg-red-500 dark:hover:text-white transition border border-red-100 dark:border-red-500/20 flex items-center gap-1">
                                                <FaBan /> Cancel
                                            </button>
                                        )}

                                        {/* Fallback for cancelled/others */}
                                        {booking.status === 'Cancelled' && (
                                            <span className="text-xs font-bold text-gray-400 dark:text-slate-600">No actions available</span>
                                        )}
